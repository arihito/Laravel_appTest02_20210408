...
<table class="table table-bordered table-striped task-table table-hover">
    ...
    <tbody> @foreach ($books as $book)
        <tr>
            ...
            <td><form action="{{url('books/'.$book->id.'/edit')}}" method="get">
                {{csrf_field()}}
            ...
            <td>
                <form action="{{url('books/'.$book->id)}}" method="post"> {{ csrf_field() }} {{ method_field('DELETE') }} <button type="method" class="btn btn-secondary">
