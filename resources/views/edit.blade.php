    ...
    <select class="custom-select" name="item_purchase">
        ...
        @for($i=1;$i<=10;$i++)
            @if(
                old(
                    'item_purchase',
                    isset($book->item_purchase) ? $book->item_purchase : ''
                    ) == $i
                )
...
<div class="form-group col-md-6 col-xs-12">
  <div class="input-group mb-2">
    ...
    <input type="date" name="published" class="form-control datetimepicker-input" value="{{ old('published', isset($book->published) ? $book->published : '') }}" id="datetimepicker" data-toggle="datetimepicker" data-target="#datetimepicker">
</div>
</div>
